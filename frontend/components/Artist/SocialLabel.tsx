...
export default function SocialLabel({ social, artist, setArtist }: SocialLabelProps) {
    const [socialDialogOpen, setSocialDialogOpen] = useState(false);

    async function removeSocial() {
        await axios
            .delete(process.env.API_URL + `/artists/${artist.id}/socials/${social.id}`)
            .then(() => {
                setArtist({ ...artist, socials: artist.socials.filter((s) => s.id != social.id) });
            })
            .catch((error) => {
                console.log(error);
            })
